...
module.exports = function (app, hexo) {
  ...
  var use = function (path, fn) {
    app.use(hexo.config.root + 'adminread/api/' + path, function (req, res) {
  ...
  use('images/upload', function (req, res, next) {
    ...
    if (settings.options) {
      ...
      imgRoot = settings.options.imgRoot ? settings.options.imgRoot : imgRoot
      imgUpMsg = settings.options.imgUpMsg ? settings.options.imgUpMsg : imgUpMsg
    }
    ...
    var publicfilename = filename
    filename = path.join(imagePath, filename)
    var outpath = path.join(hexo.source_dir, filename)

    publicfilename = path.join("../public"+imagePath, publicfilename)
    var publicoutpath = path.join(hexo.source_dir, publicfilename)
    ...
    fs.writeFile(publicoutpath, buf, function (err) {
      if (err) {
        console.log(err)
      }else{
        hexo.log.d("save success:"+publicoutpath)
      }
    })

    var joinpath = hexo.config.root+imgRoot + filename
    if (imgRoot.length > 0 && imgRoot.startsWith("http")){
      joinpath = imgRoot + filename
    }
    ...
    fs.writeFile(outpath, buf, function (err) {
      ...
      hexo.source.process().then(function () {
        res.done({
          src: path.join(joinpath),
          msg: imgUpMsg+filenamefirst
        })
      });
    })
  });
