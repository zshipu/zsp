...
module.exports = function (app, hexo) {
  ...
  app.use(hexo.config.root + 'adminread', auth(new authStrategy(hexo)));
  app.use(hexo.config.root + 'adminread/login', function (req, res) {
      if (req.method === 'POST') {
          req.authenticate(['adminAuth'], function(error, done) {
              if (done) {
                  res.writeHead(302, { 'Location':  hexo.config.root + "adminread/" });
                  res.end();
              }
          });
      } else {
  ...
  app.use(hexo.config.root + 'adminread/', function (req, res, next) {
